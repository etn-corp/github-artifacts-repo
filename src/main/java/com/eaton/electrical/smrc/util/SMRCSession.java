...
public class SMRCSession {
...

	{
		...
		InputStream inputStream = SMRCSession.class
				.getResourceAsStream("/com/eaton/electrical/smrc/conf/TapPortal.xml");
		...
		try {
			...
			/*
			 * SSOAttribute[] attributes =
			 * SSOPortalUserFactory.getSSOAttributes(request,inputStream,0);
			 * //xmlFile.delete();
			 * 
			 * 
			 * if (attributes != null){ for (int i=0; i < attributes.length; i++) {
			 * //log.debug("SSOAttribute[" + i + "] = " + attributes[i].getTagName() +
			 * " and the value is " + attributes[i].getTagValue());
			 * //System.out.println("SSOAttribute[" + i + "] = " +
			 * attributes[i].getTagName() + " and the value is " +
			 * attributes[i].getTagValue().getValue().toUpperCase()); if
			 * (attributes[i].getTagName().equalsIgnoreCase(SSOAttribute.
			 * PORTAL_COOKIE_USER_ID_TEXT)) {
			 * user.setUserid((String)attributes[i].getTagValue().getValue().toUpperCase());
			 * }
			 * 
			 * if (attributes[i].getTagName().equalsIgnoreCase(SSOAttribute.
			 * PORTAL_COOKIE_USER_FIRSTNAME_TEXT)) {
			 * user.setFirstName((String)attributes[i].getTagValue().getValue()); }
			 * 
			 * if (attributes[i].getTagName().equalsIgnoreCase(SSOAttribute.
			 * PORTAL_COOKIE_USER_LASTNAME_TEXT)) {
			 * user.setLastName((String)attributes[i].getTagValue().getValue()); }
			 * 
			 * if (attributes[i].getTagName().equalsIgnoreCase(SSOAttribute.
			 * PORTAL_COOKIE_USER_EMAIL_TEXT)) {
		...
		/*
		 * catch ( Exception e ) { SMRCLogger.error("SMRCSession.getSessionProfile() ",
		 * e) ; throw e;
		 * 
		 * 
		 * }
	...
	public static User getUser(HttpServletRequest request, boolean getNewUser, Connection DBConn)
	...
	// getUserOverride is for the UserOverride servlet to set the user based on
	// userid
